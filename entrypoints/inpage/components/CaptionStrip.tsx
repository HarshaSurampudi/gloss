...
function FullTranscript({
  ...
  // Manually scroll within the internal scroller — avoids scrollIntoView
  // bubbling up through the Shadow DOM and scrolling the YouTube page.
  useEffect(() => {
    if (query || userScrolling.current) return;
    const scroller = scrollerRef.current;
    const el = activeRef.current;
    if (!scroller || !el) return;
    const top = el.offsetTop - scroller.offsetTop - scroller.clientHeight / 2 + el.clientHeight / 2;
